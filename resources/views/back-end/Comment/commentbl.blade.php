...
<table class="table table-stripped">
    ...
    @foreach($show as $print)
            ...
            <td>{{$print->Blog->name_blog}}</td>
            <td>{{$print->content}}</td>
               <td>
                <a href="{{asset('').'admin/commentbl/delete/'.$print->id}}" class="btn btn-sm btn-danger btn-remove"><i class="fas fa-trash"></i></a>
                </td>
            </tr>
      @endforeach
        </tbody>
    ...
    <script>
        ...
        <script type="text/javascript">
            $('#search').on('keyup',function(){
                ...
                $.ajax({
                    type: 'get',
                    url: '{{ URL::to('admin/commentbl/search') }}',
                    data: {
                        'search': $value
                    },
